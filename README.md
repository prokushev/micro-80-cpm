...
средства и системы" 1984 года №4](docs/mpsis.md), необходимо программное обеспечение для
его использования. Стандарной дисковой операционной системой для МикроЭВМ
на базе Intel 8080 является CP/M. Она не была опубликована для Микро-80.
Но была опубликована для другого компьютера - ЮТ-88 в
["Юный техник. Для умелых рук" 1990 года №1](docs/ut88.md).
Данный проект посвящен реализации CP/M
...
rom-диска или с ленты. После загрузки и запуска загрузчика будет
...
## todo
 ...
 - Адаптация power.com под знакогенератор и 64 символа в строке.
...
прямое ипользование адресов точек входа. Вообще, использование BIOS не
рекомендуется.
...
Если программа размещена не в ПЗУ, то можно использовать самомодифицирующийся
код, который будет несколько ускорять повторный вызов BIOS за счет уже рассчитанного
адреса вызова.

```
; Пример вызова подпрограмм BIOS
;       До вызова подпрограммы          ; После вызова подпрограммы
	LD DE, -3			; CALL XXXX
	CALL BIOS			; NOP NOP NOP
	...
	RST 0				; Возврат в систему
...
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Перемещение головки !   21   ! ВХОДНЫЕ                      !
! диска на нулевую    !        ! ВЫХОДНЫЕ                     !
! дорожку             !        !                              !
+---------------------+--------+------------------------------+
! Выбор привода диска !   24   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Выбор дорожки       !   27   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Выбор сектора       !   30   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Установка адреса DMA!   33   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Чтение сектора      !   36   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Запись сектора      !   39   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Состояние принтера  !   42   ! ВХОДНЫЕ                      !
!                     !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
! Трансляция номера   !   45   ! ВХОДНЫЕ                      !
! сектора             !        ! ВЫХОДНЫЕ                     !
+---------------------+--------+------------------------------+
```
