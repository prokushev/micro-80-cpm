...
; todo Совместить MON и BIOS? Можно, но таблица переходов должна быть в ОЗУ, т.к. 
; некоторые программы патчат таблицу переходов (CP/NET, например)
...
;	Блок определения дисков в системе
;
...
; Для Микро-80 размер диска не 256кб, а до 512Кб. В итоге при размере блока 1024 используется двухбайтные
; указатели блоков (т.к. их 256 и больше на диске), что недопустимо. Поэтому надо увеличивать размер блока в два раза.
; Или использовать двухбайтовые номера блоков и так же править загрузчик (Сейчас он ожидает именно однобайтовые номера блоков).
; Конструктивно в Микро-80 диск формируется из блоков динамического ОЗУ по 64кб (тех же, что и базовая память),
; Так же могут быть включены блоки ПЗУ и статического ОЗУ.
